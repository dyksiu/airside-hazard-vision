...
# ====== KONFIG ======
DATA_YAML = r"C:\Users\dyksi\OneDrive\Pulpit\magisterka\yolo_test\trening\dataset_v8_naw_500\data.yaml"
SHIFT_LABELS = True   # True -> 0=tło, klasy będą 1..nc
...
def resolve_split_path(data_yaml_path: Path, data: dict, split_key: str) -> Path:
    """
    Rozwiązuje ścieżkę splitu nawet jeśli data.yaml ma wpisy typu ../train/images,
    ale fizycznie folder leży obok data.yaml jako train/images.
    """
...
def polygon_line_to_points(parts, w: int, h: int):
    ...
    Współrzędne są znormalizowane do [0,1].
    """
    if len(parts) < 7:
...
def main():
    ...
    print(f"[INFO] Liczba klas YOLO: {nc}")
    ...
    if SHIFT_LABELS:
        ...
        for i, name in enumerate(names):
            print(f"{i + 1} = {name}")
        print(f"\nDo SegFormera ustaw: NUM_CLASSES = {nc + 1}")
        print("Ustaw też CLASS_NAMES = ['background', ...]")
    else:
        for i, name in enumerate(names):
            print(f"{i} = {name}")
        print(f"\nDo SegFormera ustaw: NUM_CLASSES = {nc}")
