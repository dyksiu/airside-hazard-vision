...
MODEL_NAME = "deeplabv3_mobilenet_v3_large"  # "deeplabv3_resnet50" albo "deeplabv3_mobilenet_v3_large"
USE_PRETRAINED_BACKBONE = True

DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
PIN_MEMORY = DEVICE.type == "cuda"

RUNS_ROOT = Path(__file__).resolve().parent / "runs_deeplabv3"
RUN_NAME = "train"
...
def save_config(run_dir: Path):
    ...
    cfg.append(f"MODEL_NAME={MODEL_NAME}")
    cfg.append(f"USE_PRETRAINED_BACKBONE={USE_PRETRAINED_BACKBONE}")
...
def forward_logits(model: nn.Module, x: torch.Tensor) -> torch.Tensor:
    ...
    if isinstance(out, dict):
        if "out" not in out:
            raise RuntimeError("DeepLabV3 nie zwrócił klucza 'out'.")
        return out["out"]
    return out
...
def build_deeplabv3(num_classes: int, model_name: str, use_pretrained_backbone: bool = True):
    if model_name == "deeplabv3_resnet50":
        try:
            ...
            return deeplabv3_resnet50(
                weights=None,
                weights_backbone=ResNet50_Weights.IMAGENET1K_V1 if use_pretrained_backbone else None,
    ...
    raise ValueError(f"Nieobsługiwany MODEL_NAME: {model_name}")


# ====== DATASET ======
class SegDataset(Dataset):
    def __init__(self, images_dir: Path, masks_dir: Path, img_size: int, num_classes: int = 4):
...
def main():
    ...
    train_ds = SegDataset(train_images, train_masks, img_size=IMG_SIZE, num_classes=NUM_CLASSES)
    val_ds = SegDataset(val_images, val_masks, img_size=IMG_SIZE, num_classes=NUM_CLASSES)
