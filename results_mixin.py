        ...
        if self.results_info:
            if self.lang == "pl":
                info_text = (
                    ...
                    f"{self.tr('fps_inference')}: {runtime['inference_fps']:.1f} | "
                    f"{self.tr('fps_detection')}: {runtime['detection_fps']:.1f} | "
                    f"{self.tr('fps_display')}: {runtime['display_fps']:.1f} | "
                    f"{self.tr('infer_time_avg')}: {runtime['avg_infer_ms']:.1f} ms"
                )
            else:
                info_text = (
